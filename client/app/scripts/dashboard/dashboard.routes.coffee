angular.module('dashboard').config [
  ...
  ($routeProvider, WardenProvider) ->
    ...
    .when('dashboard.employer', user: true)
    .when('dashboard.employer.notifications', omitController: true)
    .when('dashboard.employer.completed_projects', user: true)
    .when('dashboard.employer.pending_projects', user: true)
    .when('dashboard.employer.active_projects', user: true)
